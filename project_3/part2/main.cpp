...
public:
	...
	void set_node(std::string &aux,int &pointer)
	{
		//set the name of the file
		for(int j = 0;j<8;j++)
		{
			n[i].name[j] = aux[pointer];		
			pointer++;
		}	
			
		//set the size of the file	
		n[i].size = std::stoi(aux.substr(pointer,pointer+4));
		pointer +=4;
				
		//set the pointers of the file	
		for(int k = 0;k<8;k++)
		{
			n[i].block_pointers[k] = std::stoi(aux.substr(pointer,pointer+4));
			pointer +=4;
		}
		
		//set if the file is used
		n[i].used = std::stoi(aux.substr(pointer,pointer+4));
		pointer +=4;
